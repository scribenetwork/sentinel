...
def main():
    ...
    if not is_dashd_port_open(dashd):
        print("Cannot connect to scribed. Please ensure dashd is running and the JSONRPC port is open to Sentinel.")
        return

    # check dashd sync
    if not dashd.is_synced():
        print("scribed not synced with network! Awaiting full sync before running Sentinel.")
        return
