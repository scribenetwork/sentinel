...
class DashConfig():
    ...
    def slurp_config_file(self, filename):
        # read scribe.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is scribe.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        # use default port for network if not specified in scribe.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from scribe.conf
